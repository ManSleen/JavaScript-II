...
let allCaps = [];
runners.map(x => allCaps.push(x.first_name.toUpperCase()));
console.log(allCaps);
console.log("-----------------------------------------");
...
let largeShirts = [];
runners.filter(x => x.shirt_size === "L").map(y => largeShirts.push(y));
...
// Problem 1
// We need the company names of each of the participants so we can send them a thank-you card, but we don't want any duplicates. Put all unique company names into an array called uniqueCompanyNames
let companyNames = [];
runners.map(x => companyNames.push(x.company_name));
let uniqueCompanyNames = companyNames.filter(
  (x, y, arr) => arr.lastIndexOf(x) === y
);
console.log(uniqueCompanyNames);
console.log("-----------------------------------------");

// Problem 2
// We need the emails of each runner in order to send them a notification about the race. Put all emails into an array called emailArray.
let emailArray = [];
runners.map(x => emailArray.push(x.email));
console.log(emailArray);
console.log("-----------------------------------------");

// Problem 3
// We want the initials of each runner to make them each a cake with their initials proudly drawn in frosting. Put the initials of each runner into an array called runnerInitials.
let runnerInitials = [];
runners.map(x => runnerInitials.push(x.first_name[0] + x.last_name[0]));
console.log(runnerInitials);
